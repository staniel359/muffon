...
RSpec.describe Muffon::Track do
  subject { described_class }

  describe 'successful processing' do
    context 'when track exists' do
      let(:output) {
        VCR.use_cassette 'muffon_track_success' do
          subject.call(
      ...
      it {
        expect(output).to eq(
          Helpers::WildNothing::Chinatown.muffon_track_data
        )
      }
    end
  end
  ...
  describe 'no processing' do
    context 'when no artist name given' do
      let(:output) {
        VCR.use_cassette 'muffon_track_no_artist_name' do
    ...
    context 'when no track title given' do
      let(:output) {
        VCR.use_cassette 'muffon_track_no_track_title' do
    ...
    context 'when wrong artist name' do
      let(:output) {
        VCR.use_cassette 'muffon_track_wrong_artist_name' do
          subject.call(
    ...
    context 'when wrong track title' do
      let(:output) {
        VCR.use_cassette 'muffon_track_wrong_track_title' do
          subject.call(
            artist_name: 'wild nothing',
