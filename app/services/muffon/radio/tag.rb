    ...
    private
      ...
      def track_data_hash
        ...
        sample_track.except(:mbid).tap do |h|
          ...
          h[:radio_tag] = radio_tag_name
        end
      end

      def can_proceed?
        @args.tag_name.present? && tag_artists_list.present?
      end

      def tag_artists_list
        tag_artists_data[:artists]
      end

      def tag_artists_data
        @tag_artists_data ||=
          retrieve_tag_artists_data
      end

      def retrieve_tag_artists_data
        LastFM::Tag::List::Artists.call(
      ...
      def sample_artist_name
        @sample_artist_name ||= tag_artists_list.sample
      end
      ...
      def radio_tag_name
        { name: tag_artists_data.dig(:tag, :name) }
      end
    end
  end
end
