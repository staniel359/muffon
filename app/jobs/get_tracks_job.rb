    ...
    def perform(profile_id)
      ...
      recent_tracks_pages.downto(1) do |i|
        ...
        recent_tracks.reverse.map do |t|
          ...
          profile_artist = ProfileArtist.where(artist_id: artist.id, profile: profile.id).first_or_create!(created_at: Time.zone.at(t['date']['uts'].to_i))
          profile_album = ProfileAlbum.where(album_id: album.id, profile: profile.id).first_or_create(created_at: Time.zone.at(t['date']['uts'].to_i), profile_artist_id: profile_artist.id, artist_id: artist.id) if album
          profile_track = ProfileTrack.where(track_id: track.id, profile: profile.id).first_or_create!(created_at: Time.zone.at(t['date']['uts'].to_i), loved: t['loved'].to_i, profile_artist_id: profile_artist.id, artist_id: artist.id)
          ...
          Play.where(profile_id: profile.id, track_id: track.id, profile_track_id: profile_track.id, album_id: (album ? album.id : nil), profile_album_id: (profile_album ? profile_album.id : nil), artist_id: artist.id, profile_artist_id: profile_artist.id, created_at: Time.zone.at(t['date']['uts'].to_i)).first_or_create!
