class ArtistsController < ApplicationController
	...
	def tracks
		...
		if params[:page]
			response = JSON.parse(open("http://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist=#{CGI.escape(params[:id])}&page=#{params[:page].to_i}&api_key=#{ENV["LASTFM_KEY"]}&format=json").read)['toptracks']
		else
			response = JSON.parse(open("http://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist=#{CGI.escape(params[:id])}&api_key=#{ENV["LASTFM_KEY"]}&format=json").read)['toptracks']
		end
